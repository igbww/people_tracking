...
def run_demo(opt):
    ...
    try:
        if opt.input_mode == 'video':
            ...
            output_dir = '/home/q116/anaconda3/envs/cndFairMOT/c5/results_new/results_all/tmp'
...
def test_single(img_path, dev):
    ...
    heads = {'hm': 7, 'reg': 2, 'wh': 2, 'id': 128}

    # Load model and put to device
    net = create_model(arch='resdcn_18', heads=heads, head_conv=256)
    model_path = '/home/q116/anaconda3/envs/cndFairMOT/exp/mot/default/all_res18/mcmot_last_det_resdcn_18.pth'
    ...
    with torch.no_grad():
        ...
        # Decode output
        dets, inds, cls_inds_mask = mot_decode(hm, wh, reg, 7, False, 128)

        # Get ReID feature vector by object class
        cls_id_feats = []  # topK feature vectors of each object class
        for cls_id in range(7):  # cls_id starts from 0
        ...
        for cls_id in range(7):  # cls_id start from index 0
    ...
    img_draw = plot_detects(img_0, dets_dict, 7, frame_id=0, fps=30.0)
    # cv2.imshow('Detection', img_draw)
    # cv2.waitKey()
    cv2.imwrite('/home/q116/anaconda3/envs/cndFairMOT/c5/results/00000.jpg', img_draw)
